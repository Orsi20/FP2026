import System.Win32 (LOCALESIGNATURE(lsCsbDefault), xBUTTON1)
...
myLength2 ls = foldr (\x db -> 1 + db) 0 ls  -- jobb oldali hajtogatas
...
myMinimum4 ls = foldr1 min ls  -- foldr f ke ls (meg kell adni a kezdoerteket is)  foldr1 f ls (itt nem)

--myMinimumMap = map myMinimum ls1
...
palindrom2 ls = (head ls == last ls) && palindrom2 (init $ tail ls) 
    ...
    -- ! head = legelso elem
    -- ! last = utolso elem
    -- ! init = utolso elem nelkul a lista
    -- ! tail = elso elem nelkul a lista
