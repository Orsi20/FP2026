...
main1 = do 
    ...
    putStrLn ("Legtobb valodi oszto szam: " ++ show (snd . head $ voLs) ++ ", ezzel rendelkezo szamok") -- ((snd . head) voLs)
    print (map fst voLs)
...
-- !! readLn - adott tipuskent ad at, readLn :: IO Int
...
valodiOsztok x = [i | i <- [2 .. div x 2], mod x i == 0, i /= x] -- /= nem egyenlo
